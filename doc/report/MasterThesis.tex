...
with \project{}. Next we will evaluate the choices and their consequences in section \ref{sec:evaluation}. In Section \ref{sec:future} we discuss the results based on section \ref{sec:evaluation}. We end 
...
\noindent After choosing a suitable test project we had to decide on which platforms, we should test this software. To show that \project{} runs on multiple platforms we choose to test the \emph{Marc4j} 
...
used in the sensor file, measures the build time of the CheckoutApplications project and both the checkout and build time of the \emph{Marc4j} project. There are no credentials needed for checking out the
 \emph{Marc4j} project so both the \emph{user} and \emph{pwd} fields in listing \ref{list:test:init} could be left empty.\\

\noindent Now that we have configured the tests, they are ready for execution. The results are shown in the next section.
...
<sensors>
	<onoff>
		...
		<sensor id="build_checkout_ok" name="TestApps: Build Checkout" cmd="java CODMON_HOME/codmon/wrappers/classes/TimeWrapper ../../local/checkoutApplications ant" scope="checkOut" scheduled="false" graph="true" fatal="true" />
...
issue was easy to fix by adding the expected target to the \emph{cmd}-attribute of the sensor, but it something to keep in mind. Because the test executions on both the \emph{Windows} and the \emph{Ubuntu} 
environment gave about the same results, we decided to show only the results of the \emph{Windows} environment. The results of the three tests described in section \ref{test:exec} are shown in figure 2. 
This figure is a screenshot from a page, which is generated by the \project{} environment.\\

\begin{figure}[!ht]
  \caption{\emph{Test results}}
  \centering
    \includegraphics[scale=0.5]{testResults}
\end{figure}

\newpage
\noindent The page displays a few elements. On top of the page urls are shown which navigate you to the historical results. Also a list is shown, which gives an overview of the executed tests. In this 
case the executed tests were:

\begin{itemize}
\item TestApps: Build Checkout.
