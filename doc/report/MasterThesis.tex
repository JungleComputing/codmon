...
For instance it can be a SVN command, or a ANT job or something else. Listing \ref{list:wrapper} gives an example of such a senor. \\
...
\noindent In this case the CMD attribute apparently consits of a wrapper called \emph{time\_wrapper.pl} and a shell-script command called \emph{cvsup.sh}. The wrapper indicates the kind of test
that is executed. The shell-script command indicates which programm is tested. In case of Listing \ref{list:wrapper} the update time of the CVS repository is is measured.\\
