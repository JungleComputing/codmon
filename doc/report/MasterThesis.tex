...
This chapter will give an introduction about the \project{} project by giving a brief description of background of my research and of the
...
\section{Codmon}
\label{sec:codmon}
Before we start describing the \project{} test environment, we start with a description of the current Codmon framework, we first describe how Codmon works and why it is not good enough for the purposes mentioned
in section \ref{subsec:Problemstatement}. The original Codmon framework was built in 2005 by François Lesuer\cite{Codmon}. Originally Codmon was built for testing Ibis projects\cite{Ibis}\cite{Satin}\cite{MPJ}\cite{IPL}\cite{GMI} 
...
test failed, Codmon reported the problems,by sending an email, directly to the programmer who made the last changes in code under test. It was also the intention that Codmon would be extensible. In this the Codmon 
programmers succeeded only partially. We will discuss this more in depth in section \ref{subsec:CodmonProblems}.  

\subsection{Codmon Design}
\label{subsec:CodmonDesign}
This section we will describe the design of the Codmon framework. The setup of Codmon is more or less modular and consists of a few core elements. The set of tests that should run is described
in different configuration files which are called \emph{Sensors}. A sensor describes precisely which test should be execute and also the software that is under these tests. A sensor consists of two
...
and if the performance result of a test drops below a certain level, an alarm is raised. When an alarm is raised, an email is sent to the last contributor of the code of which a test fails. 
...
language. The return value of a wrapper indicates when a test was successful or not. In case of a failure an alarm is raised and both the return value and the actual error code will be shown to
...
The goal of this research was to see if it's possible to design a multi-platform, user friendly modular test environment. There are multiple reasons why Codmon doesn't satisfy this. First
...
different languages used. The core of Codmon is written in java, which is indeed platform independent\cite{Java}, so this is not the real problem. As we've explained in section \ref{sec:codmon} the
...
not platform independent. The same can be said about the PERL language, this will,without special effort, neither work on a Windows environment. Next to this there are also several separate
scripts for example, for  CVS-checkouts and the startup of the Codmon framework. Taking this into our mind, we can easily see that the Codmon framework is far from platform independent. Due to these chaos of 
different scripts and languages it is also difficult for programmers to add new modules or tests to the Codmon framework. In The next reminder of this thesis we'll explain how \project{} solves
