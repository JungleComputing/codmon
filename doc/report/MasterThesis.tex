...
\label{test}
In this section we describe how we the tested \project{} environment with an independent software project. With the testing of an independent project in the \project{} environment, we wanted to show that \project{} is a 
\emph{multi-platform}, \emph{modular} and \emph{user-friendly} test environment.

\subsection{Test preparation}
\label{test:prep}
To see if \project{} is a \emph{multi-platform} , \emph{modular} and \emph{user-friendly} test environment we tested an existing software project in the \project{} environment. There were a few criteria 
...
Figure 2 shows the results of the run without errors on a screenshot of a browser page, which is generated by the \project{} environment. Figure 3 shows the same page, but in this case the ErrorTest is 
...
\newpage
\noindent The page displays several elements. As you can see in figure 2 and 3 a list is shown, giving an overview of the executed tests. In case of an error also the error message is displayed. In this 
