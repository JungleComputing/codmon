/* $Id: Main.java,v 1.13 2007/11/06 11:40:35 codmon Exp $ */
...
final class Main { 
	...
	try { 
	    ...
	    int i           = 0;
	    int len         = 1023;
	    int arraysizefrom = 16*1024;
	    int arraysizeto = 16*1024;
	    int count       = 10000;
	    int retries     = 10;
	    ...
	    int tests = 0;
            ...
            String imple = (System.getProperty("ibis.implementation")).substring(0,1);
		...
		if (false) {
		    ...
		    arraysizefrom = Integer.parseInt(args[i+1]);
		    arraysizeto = Integer.parseInt(args[i+2]);
		    for (int j=arraysizefrom;j<=arraysizeto;j=256*j,loops++);
		    i += 3;
		...
		java.io.FileWriter outF = null;
		if (outFile != null) {
		    outF = new java.io.FileWriter(outFile, true);
		}
		...
		if (array) { 	
		    for (int arrays = arraysizefrom; arrays <= arraysizeto; arrays = 256*arrays) {
			data = new byte[arrays];								
			if (verbose) { 
			    System.out.println("starting byte[" + arrays + "] test");
			} 

			time = runTest(rport, sport, count, retries, data, one_way, stream);
			System.out.println("byte[" + arrays + "] = " + tp((one_way ? arrays*count : 2*arrays*count), time) + " MBytes/sec.");
			if (outF != null) {
			    outF.write("<test id=\"b"+imple+(ibisSer?"i":"s")+arrays+(upcalls?"u":"d")+(stream?"s":"a")+(one_way?"o":"t")+"\" name=\"");
			    outF.write("-Dibis.implementation=" + System.getProperty("ibis.implementation"));
			    outF.write(" -bytes " + arrays);
			    outF.write(ibisSer?" -ibis":"");
			    outF.write(" -array");
			    outF.write(" -arraysize "+arrays);
			    outF.write(upcalls?" -upcalls":"");
			    outF.write(one_way?"":" -twoway");
			    outF.write(stream?" -stream":"");
			    outF.write("\" value=\"" + tp((one_way ? arrays*count : 2*arrays*count), time)*1024*1024 + "\" unit=\"Bytes/s\" />\n");
			}
		    }
			...
			time = runTest(rport, sport, count, retries, data, one_way, stream);
			System.out.println("int[" + alen + "] = " + tp((one_way ? arrays*count : 2*arrays*count), time) + " MBytes/sec.");
			if (outF != null) {
			    outF.write("<test id=\"i"+imple+(ibisSer?"i":"s")+arrays+(upcalls?"u":"d")+(stream?"s":"a")+(one_way?"o":"t")+"\" name=\"");
			    outF.write("-Dibis.implementation=" + System.getProperty("ibis.implementation"));
			    outF.write(" -int " + alen);
			    outF.write(ibisSer?" -ibis":"");
			    outF.write(" -array");
			...
			time = runTest(rport, sport, count, retries, data, one_way, stream);
			System.out.println("long[" + alen + "] = " + tp((one_way ? arrays*count : 2*arrays*count), time) + " MBytes/sec.");
			if (outF != null) {
			    outF.write("<test id=\"l"+imple+(ibisSer?"i":"s")+arrays+(upcalls?"u":"d")+(stream?"s":"a")+(one_way?"o":"t")+"\" name=\"");
			    outF.write("-Dibis.implementation=" + System.getProperty("ibis.implementation"));
			    outF.write(" -long " + alen);
			    outF.write(ibisSer?" -ibis":"");
			    outF.write(" -array");
			    outF.write(" -arraysize "+arrays);
			    outF.write(upcalls?" -upcalls":"");
			...
			time = runTest(rport, sport, count, retries, data, one_way, stream);
			System.out.println("double[" + alen + "] = " + tp((one_way ? arrays*count : 2*arrays*count), time) + " MBytes/sec.");
			if (outF != null) {
			    outF.write("<test id=\"d"+imple+(ibisSer?"i":"s")+arrays+(upcalls?"u":"d")+(stream?"s":"a")+(one_way?"o":"t")+"\" name=\"");
			    outF.write("-Dibis.implementation=" + System.getProperty("ibis.implementation"));
			    outF.write(" -double " + alen);
			    outF.write(ibisSer?" -ibis":"");
			    outF.write(" -array");
			    outF.write(" -arraysize "+arrays);
			    outF.write(upcalls?" -upcalls":"");
		...
		if (upcalls) {
		    Receiver receiver = new Receiver(null, sport, tests*retries*count*loops, one_way, false);
		    ...
		    for (int r=0;r<tests*retries*loops;r++) { 
